/*
 ...
 @ Forked and adapted for Dalecoin (DALC) by Anoxy (anoxydoxy@gmail.com)
*/
...
(function () {
	...
	//coinjs.host = ('https:'==document.location.protocol?'https://':'http://')+'dalecoin.org/api.php';
	//coinjs.host = ('https:'==document.location.protocol?'/api.php':'http://195.181.242.206:9998/api');
	//coinjs.host = ('https:'==document.location.protocol?'/api_list.php':'http://wallet.dalecoin.org/api_list.php');
	...
	//coinjs.host = ('https:'==document.location.protocol?'https://bp1.dalecoin.org/api':'http://bp1.dalecoin.org/api');
	coinjs.host = ('https:'==document.location.protocol?'https://bp2.dalecoin.org/api':'http://bp2.dalecoin.org/api');
	//coinjs.host = ('https:'==document.location.protocol?'https://bp.dalecoin.org/api':'http://bp.dalecoin.org/api');

	
	//coinjs.host = document.location.protocol + '//wallet.dalecoin.org/api.php';
	coinjs.uid = '1';
	...
	*/
	coinjs.simpleHodlAddress = function(pubkey, checklocktimeverify) {
		...
		if (coinjs.debug) {
			console.log('--------------------------simpleHodlAddress--------------------------');
			console.log('pubkey: '+pubkey);
			console.log('checklocktimeverify: '+checklocktimeverify);
		}
		...
		if (coinjs.debug) {
			console.log('------------------------------------------------------------------------');
			console.log('simpleHodlAddress s');
			console.log(s);
		}
		var x = ripemd160(Crypto.SHA256(s.buffer, {asBytes: true}), {asBytes: true});
		if (coinjs.debug) {
			console.log('x');
			console.log(Crypto.util.bytesToHex(x));
		}
		x.unshift(coinjs.multisig);
		var r = x;
		if (coinjs.debug) {
			console.log('r');
			console.log(Crypto.util.bytesToHex(r));
		}
		r = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		if (coinjs.debug) {
			console.log('r2');
		...
		if (coinjs.debug) {
			...
			console.log( Crypto.util.bytesToHex(s.chunks[3]) );
		}
		
		return {'address':address, 'redeemScript':redeemScript};
	}
	
	//p2sh script to address 
	coinjs.simpleHodlP2shAddress = function(script, pubkeys, checklocktimeverify) {
		...
		s.writeOp(177);//OP_CHECKLOCKTIMEVERIFY
		s.writeOp(117);//OP_DROP
		s.writeOp(2);//(M <-this) signature of N
			//Loop through the public keys
			for(var i=0;i<pubkeys.length;i++){
				s.writeBytes(Crypto.util.hexToBytes(pubkeys[i]));
			}
		s.writeOp(2);//M signatures of (N <- this)
		s.writeOp(174);//OP_CHECKMULTISIG


		var sha256 = Crypto.SHA256( Crypto.util.hexToBytes(script), {asBytes: true});
		
		var ripemd160_b = ripemd160( sha256, {asBytes: true});
		ripemd160_hex = Crypto.util.bytesToHex(ripemd160_b);
		
		var hashOnce = Crypto.SHA256(( Crypto.util.hexToBytes(coinjs.multisig_str+ripemd160_hex)), {asBytes: true});
		var hashTwice = Crypto.SHA256((hashOnce), {asBytes: true});

		var checksum = hashTwice.slice(0,4);
		checksum = Crypto.util.bytesToHex(checksum);
		
		var addr = (coinjs.multisig_str + ripemd160_hex + checksum);
		var addr_to_b = Crypto.util.hexToBytes(addr);
		var addr_to_hex = coinjs.base58encode(addr_to_b);

		var redeemScript = Crypto.util.bytesToHex(hashTwice);

		return ( {'address':addr_to_hex, 'redeemScript':redeemScript});
		//return {'address':address, 'redeemScript':redeemScript};
	...
	#https://bitcoin.stackexchange.com/questions/48375/p2sh-address-decoding
	Here is the example of the transaction from the question
	*/
	...
	coinjs.addressBalance = function(address, callback){
		//coinjs.ajax('http://explorer.dalecoin.org:9998/api?method=blockchain.address.get_balance&params='+address+'&r='+Math.random(), callback, "GET");
		...
		/*
				$.ajax ({
					...
					error: function(data) {
						var r = '';
						r += (data.api_status);
						r = (r!='') ? r : ' Failed to get balance!';
						console.log(r);
						//$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(r).prepend('<span class="glyphicon glyphicon-exclamation-sign"></span>');
					},
					success: function(data) {
						//if (coinjs.debug) {console.log(data)};
						console.log(data.result.confirmed);
						
						if(data.api_status=="success"){
							alert('success');
							alert(data.result.confirmed);
						} else {
							alert('failed');
						}
					}
	...
	coinjs.script = function(data) {
		...
		r.decodeRedeemScript = function(script){
			...
			try {
				...
				if (coinjs.debug) {
					console.log('---S is start----');
					console.log(s);
					console.log('---S is end----');
				}
				...
				*/
				...
				if(s.chunks.length >= 5 && s.chunks[1] == 177 && s.chunks[2] == 117 && s.chunks[7] == 174){
					// ^ <unlocktime> OP_CHECKLOCKTIMEVERIFY OP_DROP <signatures> <pubkeys> <signatures> OP_CHECKMULTISIG ^
					r = {}
					if (coinjs.debug) {
						console.log('s.chunks[4]');
						console.log(s.chunks[4]);
					}

					r.signaturesRequired = s.chunks[3]-80;
					var pubkeys = [];
					for(var i=4;i<s.chunks.length-2;i++){
						pubkeys.push(Crypto.util.bytesToHex(s.chunks[i]));
					}
					r.pubkeys = pubkeys;
					var multi = coinjs.pubkeys2MultisigAddress(pubkeys, r.signaturesRequired);
					r.address = multi['address'];
					var redeemScript = multi['redeemScript'];

					r.pubkey = redeemScript;

					r.checklocktimeverify = coinjs.bytesToNum(s.chunks[0].slice());

					//----Same  ---
					r.pubkey = coinjs.simpleHodlP2shAddress(script, r.pubkeys, r.checklocktimeverify).address;	//Hodl Address
					r.address = multi['address'];	//Signature required
					r.type = "hodl__p2sh";

				}else if((s.chunks.length>=3) && s.chunks[s.chunks.length-1] == 174){//OP_CHECKMULTISIG
					r = {};
					...
					r.type = "hodl__";
					if (coinjs.debug) { console.log('hodl__: '+r.type) };
				}
	...
	coinjs.transaction = function() {
		...
		r.listUnspent = function(address, callback) {
			//coinjs.ajax(coinjs.host+'?uid='+coinjs.uid+'&key='+coinjs.key+'&setBay=DALC&setmodule=addresses&request=unspent&address='+address+'&r='+Math.random(), callback, "GET");
			//coinjs.ajax('http://explorer.dalecoin.org:9998/api?method=blockchain.address.listunspent&params='+address+'&r='+Math.random(), callback, "GET");
		...
		r.addUnspent = function(address, callback){
			...
			this.listUnspent(address, function(data){
				...
				if((dataJSON.api_status  == "success") ){
					if (Object.keys(dataJSON['result']).length > 0){

						//Get scriptPubKey, public key has for regular address and redeemscript for multisig wallet
						
						var decode = coinjs.addressDecode(address);
						if(decode.version == coinjs.pub){ // regular address
							var s = coinjs.script();
							var scriptPubKey = s.pubkeyHash(address);
							...
							var inputScript = $("#walletKeys .redeemScript_wallet").val();
						}
						...
						alert('Unexpected error, there are no unspent outputs.');
					}
				} else {
					//$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs.');
					alert('Unexpected error, unable to retrieve unspent outputs.');
				}
				return callback(x);
		...
		r.broadcast = function(callback, txhex){
			...
			//coinjs.ajax('http://explorer.dalecoin.org:9998/api?method=blockchain.transaction.broadcast&params='+tx+'&r='+Math.random(), callback, "GET");
