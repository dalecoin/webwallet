...
$(document).ready(function() {
				...
				tx2.broadcast(function(data){
					...
					if(dataJSON.api_status=="success" || dataJSON.status){
						...
						if(success || callback_result.match(/^[a-f0-9]+$/)){
							...
							// +'<a href="http://explorer.dalecoin.org/tx/'+callback_result+'" target="_blank" >Txid: ' + callback_result + '</a>';
	...
	$("#transactionBtn").click(function(){
		...
		if(!$("#recipients .row, #inputs .row").hasClass('has-error')){
			...
			if($("#transactionFee").val()>=0.002){
				var warning = "<p>Please be aware that you have created a transaction with what seems to be a very high fee of "+$("#transactionFee").val()+" DALC!</p>"
				...
				var warning = "<p>Please be aware that you have created a transaction with what seems to be a very low fee of "+$("#transactionFee").val()+" DALC!</p>"
	...
	$("#redeemFromBtn").click(function(){
		...
		} else if(host=='explorer.dalecoin.org'){
...
/* retrieve unspent data from chain.so for Dalecoin */
...
// broadcast transaction via chain.so (mainnet)
	...
	$(".qrcodeBtn").click(function(){
		...
		if(ta.length>0){
			...
			qrstr = "dalecoin:"+$('.address',thisbtn).val();
		}
		...
		if (typeof profile_data.passwords !== "undefined" && typeof profile_data.passwords[0].password !== "undefined" && ( typeof profile_data.email !== "undefined" && typeof profile_data.wallet_type !== "undefined" && typeof profile_data.signatures !== "undefined") ){
				...
				if( checkPassword(pass) ){
					...
					//Create Regular address
						...
						$("#walletAddressExplorer").attr('href','http://explorer.dalecoin.org/address/'+address);
						$("#walletHistory").attr('href','http://explorer.dalecoin.org/address/'+address);
						...
						$("#walletQrCode").html("");
						var qrcode = new QRCode("walletQrCode");
						qrcode.makeCode("dalecoin:"+address);
